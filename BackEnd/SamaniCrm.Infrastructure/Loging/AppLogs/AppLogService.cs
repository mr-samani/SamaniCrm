...
public class AppLogService : IAppLogService
{
    ...
    public AppLogService(
        IAppLogConfigurationService configService,
    ...
    public async Task<PaginatedResult<AppLogEntryDto>> GetLogs(GetAppLogsQuery filter, CancellationToken cancellation)
    {
        ...
        return new PaginatedResult<AppLogEntryDto>(items, total, filter.PageNumber, filter.PageSize);

    }

    public async Task<AppLogEntryDto> GetLogDetails(GetAppLogDetailsQuery request, CancellationToken cancellation)
    {
        var result = await _dbContext.LogEntries
                .AsNoTracking()
                .Where(l => l.TenantId == request.tenantId && l.Id == request.logId)
                .Select(log => new AppLogEntryDto
                {
                    Id = log.Id,
                    Level = log.Level,
                    Message = log.Message,
    ...
    public async Task<AppLogStatsDto> GetStats(DateTime fromDate, DateTime toDate, Guid? tenantId, CancellationToken cancellation)
    {
