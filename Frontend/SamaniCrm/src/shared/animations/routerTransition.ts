...
export function slideFromDown() {
  return trigger('routerTransition', [
    state('void', style({ transform: 'translateY(100px) scale(.85)', opacity: '0' })),
    state('*', style({ transform: 'translateY(0px) scale(1)', opacity: '1' })),
    transition(':enter', [animate('0.3s ease-out', style({ opacity: '1', transform: 'translateY(0px) scale(1)' }))]),
  ]);
}
