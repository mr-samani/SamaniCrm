...
export class JalaliDate {
  ...
  setJalaliMonth(month: number) {
    ...
    let dayPerMonth = GregorianJalaliHelper.getDaysPerMonth(this.month, this.year);
    this.day = this.day <= dayPerMonth ? this.day : dayPerMonth;
    return this;
  }
...
class GregorianJalaliHelperClass {
   ...
   * utc to local
   */
  fromGregorian(gdate: Date | DateTime): JalaliDate {
    if (gdate instanceof DateTime) {
      const g2d = this.gregorianToDay(gdate.year, gdate.month, gdate.day);
...
function stringReplaceBulk(str: string | undefined, findArray: string[], replaceArray: (string | number)[]) {
  if (!str) return '';
  const regex: string[] = [];
  const map: any = {};
...
export const NARROW_MONTHS = ['فر', 'ار', 'خر', 'تی', 'مر', 'شه', 'مه', 'آ', 'آذ', 'دی', 'به', 'اس'];
