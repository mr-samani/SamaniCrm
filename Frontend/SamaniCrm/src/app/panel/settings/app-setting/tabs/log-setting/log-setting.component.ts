...
export class LogSettingComponent extends AppComponentBase implements OnInit {
  ...
  setErrorsAndInfo() {
    this.enabledLevels = [LogLevelFlag.Information, LogLevelFlag.Warning, LogLevelFlag.Error, LogLevelFlag.Critical];
  }
  setAllLogLevels() {
    this.enabledLevels = [
      LogLevelFlag.Trace,
    ...
    this.logService
      ...
      .subscribe((response) => {
        ...
        if (settings?.enabledLevels) {
          this.enabledLevels = Bitmask.getActiveFlagInfos(settings.enabledLevels, LogLevelFlag);
        } else {
          this.enabledLevels = [];
        }
        if (settings?.enabledSinks) {
          this.enabledSinks = Bitmask.getActiveFlagInfos(settings.enabledSinks, LogSinkFlag);
        } else {
          this.enabledSinks = [];
        }
      });
